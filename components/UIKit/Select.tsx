...
interface Props<T> {
  ...
  options: { label: string; value: T; icon?: React.FC<any> }[];
  className?: string;
}
    ...
    <div className={className}>
      <Listbox value={value} onChange={onChange}>
        <div className="relative mt-1">
          <Listbox.Button className=" mt-2 relative w-full cursor-default rounded-lg bg-elevation-3 hover:bg-elevation-4 py-2 pl-3 pr-10 text-left shadow-md focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-orange-300 ">
...
export function SelectButton({ selected, className }: ButtonProps) {
  return (
      ...
      {({ selected }) => (
        <>
          <div className="flex flex-row items-center justify-start px-4 py-2 ">
            {children}
            {Icon && (
              <span className={`inline ${selected ? "text-light-100" : "text-light-200"}`}>
